...
func(s *Server)create(w http.ResponseWriter,r *http.Request){var e store.Link;json.NewDecoder(r.Body).Decode(&e);if e.Slug==""{we(w,400,"slug required");return};s.db.Create(&e);wj(w,201,s.db.Get(e.ID))}
...
func(s *Server)stats(w http.ResponseWriter,r *http.Request){wj(w,200,map[string]int{"links":s.db.Count()})}
func(s *Server)health(w http.ResponseWriter,r *http.Request){wj(w,200,map[string]any{"status":"ok","service":"lasso","links":s.db.Count()})}
