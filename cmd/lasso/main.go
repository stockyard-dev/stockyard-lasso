// Stockyard Lasso — Link shortener.
// Shorten URLs, track clicks, custom slugs, vanity redirects. Self-hosted.
...
import (
	...
	"github.com/stockyard-dev/stockyard-lasso/internal/license"
	"github.com/stockyard-dev/stockyard-lasso/internal/server"
...
func main() {
	...
	licenseKey := os.Getenv("LASSO_LICENSE_KEY")
	licInfo, licErr := license.Validate(licenseKey, "lasso")
	...
	if licInfo != nil && licInfo.IsPro() {
		...
		log.Printf("  License:   Free tier (set LASSO_LICENSE_KEY to unlock Pro)")
	}
	...
	log.Printf("")
	log.Printf("  Stockyard Lasso %s", version)
	log.Printf("  Redirect:       http://localhost:%d/{slug}", port)
	log.Printf("  API:            http://localhost:%d/api/links", port)
	...
	go func() {
		for {
			...
			if err != nil {
				...
				log.Printf("[cleanup] deleted %d click records older than %d days", n, retentionDays)
			}
		}
	}()
